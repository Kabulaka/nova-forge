...
需求阶段按统一 SOP 生成 `stageProjection`：`inheritedContracts` 只引用已确认的总体业务范围、既有需求版本和与当前增量相关的正式业务契约；`stageEvidence` 只保存本阶段查明的业务或项目事实；`stageDecisions` 只保存本阶段新增且已披露的业务候选。需求阶段不继承或写入架构选型和功能实现决定；增量证据与既有需求冲突时，在需求权威内重新确认受影响契约，无冲突内容保持有效。
