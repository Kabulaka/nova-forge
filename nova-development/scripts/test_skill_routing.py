#!/usr/bin/env python3
"""Contract tests for nova-development context and resource routing."""
...
"""
    ...
    def test_skill_name_and_review_boundary_are_explicit(self) -> None:
        self.assertIn("name: nova-development", SKILL)
        self.assertNotIn("name: project-brainstorming", SKILL)
        self.assertIn("不用于总体产品需求、共享技术架构、独立 Review", SKILL)
    ...
    def test_document_examples_are_scoped_to_project_first_creation(self) -> None:
        self.assertIn("目标项目不存在 `.nova/PROJECT_BLUEPRINT.md` 且正在首次创建蓝图时", SKILL)
        self.assertIn("`.nova/design/` 不存在有效设计且正在首次创建设计时", SKILL)
        self.assertIn(
            "[完整蓝图示例](references/examples/equipment-borrowing/.nova/PROJECT_BLUEPRINT.md)",
            SKILL,
        )
        self.assertIn(
            "[关联设计示例](references/examples/equipment-borrowing/.nova/design/2026-08-25_设备借用闭环.md)",
            SKILL,
        )
    ...
    def test_stage_projection_is_visible_and_authority_preserving(self) -> None:
        ...
        self.assertIn("按 `inheritedContracts` 的真实来源留在需求权威内", authority)
        self.assertIn("进入当前阶段前已正式确认", authority)
        self.assertIn("需求阶段可继承同一需求权威内", authority)
        self.assertIn("不得把同阶段来源伪装为上游阶段", authority)
