...
function pair(value, host, cwdHash, now) {
  prune(value, now);
  const instances = listJson(value.instances)
    .map((file) => ({ file, record: readJsonFile(file) }))
    .filter(({ record }) => record.host === host && record.cwdHash === cwdHash);
  const claims = listJson(value.claims)
    .map((file) => ({ file, record: readJsonFile(file) }))
    .filter(({ record }) => record.host === host && record.cwdHash === cwdHash);
...
export function registerMcpInstance(
  ...
  return { instanceId, capability, cwdHash, host, pid, outcome };
}
...
export function consumeBinding(
  ...
  return withLock(dataRoot, registration.host, (value) => {
    ...
    if (
      ...
      binding.cwdHash !== registration.cwdHash ||
