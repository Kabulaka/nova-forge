...
对已存在蓝图，在读取前后分别计算 SHA-256；两次不一致时丢弃正文并重新读取，连续变化无法取得稳定快照时停止并报告，不绑定旧理解与新哈希。首次向用户提问前，完整读取一次 [访谈 SOP](references/conversation-sop.md) 并记录其 SHA-256。上下文丢失时按首次进入处理，不猜测旧状态。
