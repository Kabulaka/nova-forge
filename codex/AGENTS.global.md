...
1. 上下文即将压缩、正在生成压缩摘要或压缩后续接时，优先保留当前目标与阶段、用户已确认决定、明确排除、委托范围、AI 候选身份、当前问题、活动交付范围、有效证据、文件与提交状态、下一动作，以及已加载控制文档的绝对路径与 SHA-256。
2. 压缩后先沿用摘要继续，不得把已回答事项重置为待确认，也不得把候选决定提升为用户确认或把暂存范围提升为当前范围。
