...
3. context-mode 只打印结论和必要证据，默认不超过 40 行或 4KB；禁止输出完整 `FILE_CONTENT`。
4. 三个以上相关读取使用 `ctx_batch_execute`；多个检索问题合并到一次 `ctx_search`。
5. `ctx_execute_file` 只读取项目根内文件；项目外文件使用宿主允许的精确读取，失败后不得换方式绕过边界。
6. 查询延迟工具时只提取目标工具的参数声明，每个工具每个会话最多查询一次，不输出完整 `ALL_TOOLS` 描述。
