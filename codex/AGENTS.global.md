...
8. 已命中技能后必须完整读取的 `SKILL.md` 及其直接引用的控制文档属于指令加载，不按普通大文件分析处理；首次使用宿主允许的精确读取完整加载，不得先 `wc`、预设固定行窗口或用 `ctx_execute_file` 摘要替代正文；只有工具明确截断时才从未返回位置续读。同一会话内记录资源绝对路径与 SHA-256，未变化时复用；不得因 UI 的重复 `Read` 或 `PostToolUse` 展示再次读取。
