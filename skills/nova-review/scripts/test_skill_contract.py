...
class NovaReviewSkillContractTests(unittest.TestCase):
    ...
    def test_commit_contract_has_stable_identity_and_fail_closed_exemptions(self) -> None:
        ...
        self.assertIn("一个独立、内聚、可暂停、可取消且有完整验收的交付结果", COMMIT)
        ...
        self.assertIn("type(scope): 中文结果摘要", COMMIT)
    ...
    def test_review_and_audit_rules_are_routed_not_duplicated(self) -> None:
        ...
        self.assertIn("| `review` | Review 完成报告 |", IMPLEMENTATION)
        self.assertIn("轮次为“不适用”", IMPLEMENTATION)
        self.assertIn("完整 diff 命中证据", IMPLEMENTATION)
        self.assertIn("`--emit-report`", IMPLEMENTATION)
        self.assertIn("最终回复必须完整、原样使用这份输出", IMPLEMENTATION)
        self.assertIn("不得重新概括、改写、删减章节", IMPLEMENTATION)
        self.assertIn("按阶段实际发生顺序完整、原样拼接", IMPLEMENTATION)
        ...
        self.assertIn("review_heads", AUDIT)
        self.assertIn("closure commit 的唯一父提交", AUDIT)
        self.assertIn("closure parent 的 Git 快照", AUDIT)
        self.assertIn("完整 staged binary diff 摘要", AUDIT)
    ...
    def test_pass_completion_report_routes_to_review_specific_structure(self) -> None:
        ...
        self.assertIn("closure commit 完成且 `query` 可信校验通过后", SKILL)
        self.assertIn("完整读取整个", SKILL)
        self.assertIn("使用第 7 节的 `review` 专用结构", SKILL)
        self.assertIn("每轮问题及修正", SKILL)
        self.assertIn("不得复用 FEAT 开发报告", SKILL)
        self.assertIn(
        ...
        for section in (
            ...
            self.assertIn(section, IMPLEMENTATION)
