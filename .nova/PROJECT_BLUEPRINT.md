...
| C-22 | Codex 与 Claude Code 的用户级全局规则入口解析到同一份工作区权威规则；双宿主安装必须先完成全部目标预检，已有软链接只解除链接本身并重新建立，普通文件或真实目录一律拒绝覆盖，任一失败不得遗留部分切换 | 隔离用户目录中的首次安装、重复安装、正常与失效软链接、普通文件、真实目录、中途失败回滚及链接解析测试 |
| C-23 | 压缩提示应保留当前任务、决定、范围、证据、文件与提交状态、下一动作及已加载控制内容的绝对路径和指纹；压缩后复用未变化内容，只在当前动作需要逐字正文而摘要不足时精确恢复；该约束不借助 Hook 或独立状态系统，验收不得承诺宿主绝不遗漏 | 普通任务与 Nova 任务的压缩前后对照、控制内容读取记录、缺失信息定向恢复及能力边界检查 |
