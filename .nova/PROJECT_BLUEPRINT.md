...
| 发现入口 | 本地文件系统符号链接；Codex 与 Claude Code 用户级规则入口 | 个人技能目录及两个宿主的全局规则链接状态 |
...
| `codex/` | Codex 与 Claude Code 共用的全局治理及安装工具 | 权威规则只保留一份；通过两个宿主的用户级符号链接发现，不创建宿主专用副本 |
...
| C-22 | Codex 与 Claude Code 的用户级全局规则入口解析到同一份工作区权威规则；双宿主安装必须先完成全部目标预检，规范入口和 `project-brainstorming`/`nova-brainstorming` 旧别名的软链接只解除链接本身，旧别名不再重建，普通文件或真实目录一律拒绝覆盖或清理，任一失败不得遗留部分切换 | 隔离用户目录中的首次安装、重复安装、正常与失效软链接、旧别名清理、普通文件、真实目录、中途失败回滚及链接解析测试 |
| C-23 | 压缩提示应保留当前任务与阶段、已确认决定、排除、委托与候选身份、未决差量、当前问题、活动范围、证据、文件与提交状态、下一动作及已加载控制内容的绝对路径和指纹；压缩后按用户明确指令、项目规则、全局默认的顺序复用未变化内容，只在当前动作需要逐字正文而摘要不足时精确恢复；该约束不借助 Hook 或独立状态系统，验收不得承诺宿主绝不遗漏 | 两个宿主真实规则加载、至少一次宿主真实压缩前后对照、普通任务与 Nova 任务静态契约、控制内容读取记录、缺失信息定向恢复及能力边界检查 |

### 开发决策边界

| 边界 | 内容 |
|------|------|
| 本期必须实现 | 需求—架构—开发分层、`.nova` 统一布局、有限加载、并行契约门禁、默认快速提交、人工 Review/分片审计，以及 Codex/Claude Code 共用的压缩保留规则与安全安装入口 |
| 明确不做 | 不建设技能市场、远程发布、遥测、账号系统、第二份个人目录实体源码、压缩 Hook、恢复数据库或会话状态服务 |
...
| PEND-005 | P1 | 历史迁移 | 双宿主全局规则与安装 | [双宿主压缩续接 WP-01](design/2026-09-02_双宿主压缩续接.md#wp-01-dual-host-continuity) | 无 | Codex 与 Claude Code 加载同一份压缩保留规则，重复安装可安全刷新软链接且不覆盖已有普通文件或目录 | REQ-01a0621d-6904-7a4f-bd7e-4552e3c8f994@v1 |
...
| 执行层 | 执行确定性校验、转换或安全安装 | `skill-name/scripts/`、`codex/scripts/` | 指令契约和被验证输入，不依赖宿主运行时服务 |
| 接口层 | 暴露 UI 元数据与发现入口 | `skill-name/agents/`、个人目录链接、Codex/Claude Code 用户级规则入口 | 指令层，不复制业务规则 |
...
| 发现链接切换 | 技能维护者 | 全部目标预检后再切换；已有软链接只解除链接本身并重建，目标异常或中途失败时停止并恢复本次已改变的链接，不留下第二份实体源码 |
