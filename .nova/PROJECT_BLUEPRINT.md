...
| C-19 | `REQ-{uuidv7}` 是长期需求身份并以版本和 `待实现/开发中/已实现/已更新` 管理，`PEND-*` 是独立交付任务身份，内部里程碑没有独立 Review 身份；交付需求实现的版本 5 设计和活动蓝图必须保存逐字一致的 `REQ-...@vN`，仅交付可独立复用架构且不得回写需求状态的 PEND 必须在两处逐字一致写 `无` 并由架构硬依赖保存 `REQ-...` 溯源；开发中时“已实现版本/实现依据”保留上一完整实现或同时为“无”，进度只记台账，全部有效任务 PASS 后才以当前版本和全部可信依据原子置为已实现 | 四状态需求校验、跨文档引用不变量、架构前置溯源、多任务 Review 聚合和状态回写测试 |
...
| C-32 | 架构只在共享工程骨架、数据所有权、公共 API、事件或 Mock 变化时进入；可独立复用的架构形成独立 PEND 并在下游任务前从可信审计派生 PASS，仅服务一个规模可控任务的架构作为同一 PEND 内部门禁并由该任务最终 Review 验证，任何未审架构均不得对外宣称 ready | 架构跳过与进入判定、独立性判定、同任务门禁、ready 状态和过度拆分负例 |
| C-33 | 首个开发任务前，每个 `REQ@版本` 必须有一份完整严格 JSON 台账，一次性登记全部稳定 PEND、各任务内部里程碑、依赖、可执行完成定义和逐字 Requirement-Ref；delivery-plan 检查点成功后需求进入开发中，详细设计可在逐项激活时补齐 | 台账 schema、任务与里程碑完整性、依赖图、需求交叉引用、计划提交和首项准入测试 |
| C-34 | 单个 PEND 的全部 required 提交只在整体完成定义满足后统一 Review，可信 PASS 才更新任务进度；只有当前需求版本全部有效 PEND 均 PASS 且当前、剩余和阻塞为空时需求才已实现，并保存全部可验证实现依据；Review 发现不得选择 requirement、delivery-plan 或内部里程碑 | 多任务顺序关闭、单任务多提交、可信审计、聚合状态、实现依据与 Review 选择正反用例 |
| C-35 | 已登记任务和内部里程碑不得静默删除；任务的新增、拆分、合并、替代、重排、激活、设计绑定、提交 Review、完成、取消和阻塞，以及里程碑的状态变化都必须递增台账计划版本并以受限 kind 保留身份、关系和原因；拆分必须验证独立、内聚、可控边界，减少或无法证明等价的业务验收返回需求阶段升级版本；所有入口固定展示任务和当前里程碑进度 | 完整状态迁移、过度拆分拒绝、计划演进、范围减少拒绝、历史保留、跨会话恢复、稳定排序和进度测试 |
...
| 交付台账 | 每个需求版本恰有一份严格 JSON 权威台账；蓝图只投影未完成有效任务，内部里程碑不形成独立 PEND，任务完成只从可信 Review 审计派生，计划变化保留原身份与原因 | schema、任务边界、里程碑、交叉引用、依赖无环、历史比较、并发写入和投影一致性测试 |
