...
| C-19 | `REQ-{uuidv7}` 是长期需求身份并以版本和 `待实现/开发中/已实现/已更新` 管理，`PEND-*` 是独立交付身份；交付需求实现的版本 5 设计和活动蓝图必须保存逐字一致的 `REQ-...@vN`，仅交付需求实现前置架构且不得回写需求状态的 PEND 必须在两处逐字一致写 `无` 并由架构硬依赖保存 `REQ-...` 溯源；开发中时“已实现版本/实现依据”保留上一完整实现或同时为“无”，进度只记台账，全部有效切片 PASS 后才以当前版本和全部可信依据原子置为已实现 | 四状态需求校验、跨文档引用不变量、架构前置溯源、多切片 Review 聚合和状态回写测试 |
...
| C-35 | 已登记切片不得静默删除或复用；新增、拆分、合并、替代、重排、激活、设计绑定、提交 Review、完成、取消和阻塞变化必须递增台账计划版本并以受限 kind 保留原身份、关系和原因，减少或无法证明等价的业务验收必须返回需求阶段升级版本；所有入口固定展示总数、已完成、当前、剩余和阻塞 | 完整状态迁移、计划演进、范围减少拒绝、历史保留、跨会话恢复、稳定排序和五项进度测试 |
...
| PEND-01a06a50-27d0-7fe5-8d30-ab9de658eaa0 | P0 | 用户提出 | 需求检查点与bootstrap状态门禁 | [.nova/design/2026-09-04_需求检查点与bootstrap状态门禁.md#wp-01-requirement-checkpoint-bootstrap-gate](design/2026-09-04_需求检查点与bootstrap状态门禁.md#wp-01-requirement-checkpoint-bootstrap-gate) | 无 | 确认需求可立即形成独立 requirement commit；需求规范、索引校验和 Review 关闭支持开发中；仍有有效切片时只更新进度，首片 PASS 不会提前置为已实现 | REQ-01a06a50-2732-704d-97d0-7a98b205a4ea@v1 |
| PEND-01a06a50-281a-74ce-97a5-74e0385625e7 | P0 | 用户提出 | 完整交付台账与最终状态聚合 | 待澄清 | 无 | 首片前完整登记切片并迁移 bootstrap 台账；全部状态迁移留痕，只有全部有效切片 PASS 后需求才以全部依据置为已实现 | REQ-01a06a50-2732-704d-97d0-7a98b205a4ea@v1 |
| PEND-01a06a50-2864-7a54-a080-9c0c2e4385e6 | P1 | 用户提出 | 技能流程、进度报告与端到端验证 | 待澄清 | 无 | 需求、架构、开发和 Review 全链路执行新门禁，并固定展示总数、已完成、当前、剩余和阻塞 | REQ-01a06a50-2732-704d-97d0-7a98b205a4ea@v1 |
