...
| C-20 | 绿地项目先需求后架构；共享工程骨架、数据所有权及所需 API/事件/Mock 契约存在真实差量时，必须由已提交 ARCH 检查点绑定且字节未漂移后才启动并行业务开发；schema 1 的既有 PEND Review 架构证据只作历史兼容，只有不可消除的业务硬依赖允许串行 | 架构 ready 门禁、ARCH 归属与字节绑定、历史兼容、契约内容和并行场景测试 |
...
| FEAT-01a06ba4-274d-7725-8788-d99d322f42d2 | 待Review | P1 | 用户提出 | 交付任务分类、阶段报告与代码化门禁 | [WP-01](design/2026-09-04_交付治理分类与报告闭环.md#wp-01-delivery-governance-closure) | 无 | 新编号、最小状态、专用报告、提交与 Review 单闭环均由技能和脚本一致执行，Doctor 可只读发现偏移 | [REQ-01a06a50-2732-704d-97d0-7a98b205a4ea@v3](requirements/REQ-01a06a50-2732-704d-97d0-7a98b205a4ea_需求基线与完整切片跟踪.md) |
