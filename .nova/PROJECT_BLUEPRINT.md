...
| C-09 | 外部参考事实、AI 候选决定和用户正式确认必须分离；阶段切换只继承上游正式契约，用户可见摘要必须区分继承契约、本阶段事实、本阶段新增候选和未决差量，零差量说明消解依据但不制造问题，最终确认仍覆盖整份阶段契约 | 三阶段路由、阶段差量投影、参考研究与候选修正前向测试 |
...
| PEND-01a0654f-7c75-779b-8ef0-d9ecc6211322 | P1 | 用户提出 | 三阶段访谈差量投影架构 | [阶段差量投影架构 WP-01](design/2026-09-03_三阶段访谈差量投影架构.md#wp-01-stage-delta-projection-architecture) | 无 | 蓝图、工程骨架和决策状态契约共同冻结可验证的阶段差量投影与恢复边界 | 无 |
