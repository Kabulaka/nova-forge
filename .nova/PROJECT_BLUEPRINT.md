...
| C-19 | `REQ-{uuidv7}` 是长期需求身份并以版本和 `待实现/已实现/已更新` 管理，`PEND-*` 是独立交付身份；设计和活动蓝图只通过 `REQ-...@vN` 引用 | 需求校验、设计元数据、蓝图引用和 Review 状态回写测试 |
| C-20 | 绿地项目先需求后架构；共享工程骨架、数据所有权及所需 API/事件/Mock 契约 Review PASS 后才启动并行业务开发，只有不可消除的业务硬依赖允许串行 | 架构 ready 门禁、契约索引与并行场景测试 |
| C-21 | 项目治理文档统一位于 `.nova/`；旧布局迁移先 dry-run 再经明确批准用 `git mv` 原子应用，历史审计和终态设计正文保持字节不变并通过确定性旧路径映射解析 | 迁移冲突、零写入、兼容解析与 Git diff 测试 |
...
| PEND-01a05677-dd33-764c-b3e2-60051401f7b8 | P1 | 用户提出 | Nova需求—架构—开发流程 | [WP-01](design/2026-08-31_Nova需求架构与开发流程.md#wp-01-nova-delivery-flow) | 无 | 需求、架构、开发和Review按统一 `.nova` 契约完成路由、迁移、并行准备与有限加载验收 | 无 |
