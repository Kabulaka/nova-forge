...
| 结构化权威状态 | 当前会话中的 AI 经宿主 `PreToolUse` 为本次调用注入一次性可信作用域证明后，由最小 MCP 工具提交 | 同宿主同会话 Hook、MCP 查询与恢复流程 | 从 transcript、压缩摘要或未经标记的自然语言自动推断正式决定，或由模型指定其他 host/sessionId、生成作用域证明 |
...
| scopeBinding | 宿主适配器从 Hook 公共输入建立的只读当前 host、sessionId 摘要和目录摘要 | 只作为状态核心的查询/写入边界；写入/查询的模型声明参数不接受 host 或 sessionId，缺失、错配或跨边界回退一律拒绝 | 新会话产生新摘要；不落入模型可选参数，原始会话 ID 不落盘 |
| scopeProof | `PreToolUse` 为单次本插件工具调用随机生成的不透明句柄及用户私有根中的短期记录 | 记录绑定 host、sessionKey、cwdHash、工具名、toolUseId、签发时间和过期时间；Hook 覆盖任何调用方同名字段，MCP 持锁原子消费后才返回 scopeBinding；缺失、伪造、过期、错配或重放均零状态访问 | 不进入 checkpoint、恢复胶囊或诊断正文；宿主可能记录的已消费句柄不得再次使用，未消费记录到期清理；升级后旧会话需新开以启用 `PreToolUse` |
...
| stateRoot | 默认用户主目录下 `.nova`，或规范化绝对 `NOVA_HOME`；目录权限和真实可写探针必须通过 | 状态、一次性证明记录、迁移记录和临时文件均不得逃出规范化根；不得以项目目录、系统临时目录或插件缓存静默替代 | 根位置改变必须通过显式覆盖或确定性迁移，不得按可写性猜测多个候选根 |
...
| Hook、MCP 与租约并发 | `PreToolUse` 证明记录按 host 与随机句柄原子创建/消费，状态核心再按证明解析出的可信作用域串行化 envelope 变更 | 同 PID 多会话、同目录多会话及 fork/子代理不按 MCP 进程数量配对；authority 写入重读并合并更高 leaseVersion，租约刷新不得覆盖更高 authorityGeneration | 同一证明最多消费一次；同一事件 ID 或租约活动 ID 重复送达只记录一次 |
...
| 作用域证明缺失、错配、伪造、过期或重放 | MCP 在读取任何会话状态前拒绝调用并记录无敏感值诊断 | 后续本插件工具调用由当前宿主 `PreToolUse` 重新签发单次证明；不得接受模型提供的替代键、旧句柄或进程数量猜测 | 宿主适配器与 MCP |
