...
| 门禁 | 是否需要 | 状态 | 确认依据 |
|------|----------|------|-------------|
| 共享工程骨架 | 是 | 已确认 | ARCH-01a094b6-9fbc-79fc-a949-8e221f78454f |
| 数据所有权与契约 | 是 | 已确认 | ARCH-01a094b6-9fbc-79fc-a949-8e221f78454f |
| 公共 API 契约 | 否 | 不适用 | 无 |
| 事件契约 | 否 | 不适用 | 无 |
| Mock 与测试夹具 | 否 | 不适用 | 无 |
...
| 工程骨架 | Codex 与 Claude Code 插件分发、状态初始化、一次性调用作用域证明、异常降级及同会话续接 | [双宿主插件工程骨架](foundation/dual-host-plugin.md) | 已确认 | 插件运行时 |
| 数据 | 用户私有根中的双宿主隔离检查点、一次性作用域证明与无权威降级 | [会话检查点数据契约](data/session-checkpoint.md) | 已确认 | 状态核心 |
