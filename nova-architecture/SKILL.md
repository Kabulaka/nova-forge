...
架构阶段按统一 SOP 生成 `stageProjection`：`inheritedContracts` 只引用与当前范围相关的已确认需求；`stageEvidence` 只保存本阶段查明的技术栈、蓝图、代码、配置和外部技术事实；`stageDecisions` 只保存本阶段新增且已披露的共享架构候选。不得把需求契约复制成架构决定；架构证据与继承需求冲突时，暂停架构收敛并返回 `inheritedContracts` 标明的需求来源重新确认，不在架构阶段改写业务语义。
