...
import { validateCiWorkflow } from "./workflow-policy.mjs";
...
for (const command of ["scripts/validate-release-notes.mjs", "npm run check", "npm pack --ignore-scripts", "scripts/checksum.mjs", "gh release create"]) {
  requirePattern(release, new RegExp(command.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), `release missing ${command}`);
}
requirePattern(release, /--notes-file/, "release must publish committed release notes");
forbidPattern(release, /--generate-notes/, "release must not replace curated notes");
