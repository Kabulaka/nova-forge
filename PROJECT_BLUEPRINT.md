...
> 事实来源：待开发目标以 `docs/design/` 工作包为准；当前行为以技能源码、脚本和测试为准。
...
| 审计记录 | JSONL 与严格 JSON-in-YAML | `docs/audit/features/`、`docs/audit/reviews/` 与 `docs/audit/index/` |
...
```text
PROJECT_BLUEPRINT.md                 工作区公共契约和待办索引
codex/AGENTS.global.md               全局 Codex 最小启动与安全内核
docs/design/                         已确认或已实现的史诗设计
docs/audit/                          按年/月及工作项哈希分片的功能、Review 与索引审计
skill-name/
├── SKILL.md                         技能入口和资源路由
...
| 代码区域 | 职责 | 代码落位规则 |
|----------|------|--------------|
| 根目录 | 工作区公共治理 | 只放蓝图和仓库级控制文件，不复制技能内部规范 |
| `docs/design/` | 工作区史诗设计 | 一个史诗可含多个相关工作包，已完成设计继续保留 |
| `docs/audit/` | Review 与完成功能审计 | 年度功能 JSONL、月度严格 JSON-in-YAML、工作项哈希索引，不建立无限增长总账 |
...
| C-09 | 外部参考事实有来源，与适配建议和用户确认要求分离 | 参考研究前向测试 |
...
| C-13 | 工作项身份与 commit 证据分离；完成和 Review 按年度 JSONL、月度严格 JSON-in-YAML 与工作项哈希索引分片记录，非法、伪造、跨年重复或并发关闭失败封闭 | trailers、审计一致性、定向查询、竞态与幂等关闭测试 |
| C-14 | 新工作项使用分类前缀加小写规范 UUIDv7 无状态生成；既有数字编号保持合法且不改写，Review 始终沿用原 ID | UUID 版本/variant、兼容性、非法格式和唯一性测试 |
...
| 边界 | 内容 |
|------|------|
| 本期必须实现 | 技能唯一源码、渐进披露、蓝图/设计/实现三层事实来源、默认快速提交和人工 Review/分片审计 |
| 明确不做 | 不建设技能市场、远程发布、遥测、账号系统或第二份个人目录实体源码 |
| 后续候选 | 仅限第 6 节尚未完成的工作包，澄清前不获得实施授权 |
| AI 可自行决定 | 不改变触发、外部行为、安全和数据语义的内部命名、排版及脚本组织 |
| 必须再次确认 | 删除或重命名技能、改变公开调用、引入依赖/网络/凭据、配置远程仓库、改变发现链接策略或放宽校验 |

## 6. 待开发功能

| 编号 | 优先级 | 来源 | 功能 | 设计依据 | 前置依赖 | 完成定义 |
|------|--------|------|------|----------|----------|----------|
| PEND-002 | P2 | 历史迁移 | 工作区统一验证入口 | 待澄清 | 待澄清 | 一条本地命令可发现全部技能并分别执行结构与行为校验 |
| PEND-003 | P2 | 历史迁移 | 技能目录索引 | 待澄清 | 待澄清 | 自动生成技能名称、用途、入口和验证状态，不复制技能正文 |
| PEND-004 | P3 | 历史迁移 | 持续集成 | 待澄清 | 待澄清 | 在受控环境执行无网络的结构、语法和正反用例检查 |
| PEND-005 | P3 | 历史迁移 | 安装与回滚工具 | 待澄清 | 待澄清 | 安装入口切换可验证、可回滚，且不覆盖用户已有技能 |
| PEND-01a04beb-9112-7e64-b320-67f42635f25b | P0 | 问题诊断 | Context-mode 有界输出与精确探测 | [WP-01](docs/design/2026-08-29_Context-mode有界输出与精确探测.md#wp-01-bounded-context-mode) | 无 | 已知工具不探测，未知工具只精确探测一次；进入模型上下文的结果不超过 40 行或 4KB，违规可由只读会话审计器确定性发现 |
| PEND-01a04c03-4b9d-7756-8557-550bf838bb84 | P0 | 问题诊断 | 显式收敛确认门禁 | [WP-01](docs/design/2026-08-29_显式收敛确认门禁.md#wp-01-explicit-convergence-confirmation) | 无 | 未决差量不能因讨论或开发请求被跳过；整份设计只需一次明确确认，确认后自动实施；中断、拒绝及语义指纹失配均保持未确认 |
...
| 层 | 职责 | 对应代码结构 | 允许依赖 |
|----|------|--------------|----------|
| 治理层 | 定义工作区公共契约和未完成工作 | `PROJECT_BLUEPRINT.md`、`docs/design/` | 不依赖单个技能实现细节 |
