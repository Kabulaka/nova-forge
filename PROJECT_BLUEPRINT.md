...
| C-07 | Git 只提交精确范围和已验证变更；失败提交不改变已提交基线，恢复使用可审计的新提交 | `git status`、提交历史和恢复演练 |
| C-08 | 已完成工作包从蓝图删除，设计文档及稳定锚点继续保留 | 蓝图与设计生命周期校验 |
...
| 边界 | 内容 |
|------|------|
| 本期必须实现 | 技能唯一源码、渐进披露、结构与行为验证、蓝图/设计/实现三层事实来源 |
...
| Review 拒绝 | 主代理 | 统一修复全部 Blocker 和 Observation-Fix，重测后增量复审 |
